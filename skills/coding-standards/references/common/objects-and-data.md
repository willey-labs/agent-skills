...
**Carve-out.** Generated, vendored, and third-party stub code you don't own is out of scope (the skill skips it). For code you *do* own, the write-time hook blocks **every** `any` / `Any` / `interface{}` / `dynamic` / `mixed` — there is **no inline-comment escape**, so a commented `any` still blocks. If a value is genuinely untyped at a third-party boundary, narrow it from `unknown` (the rule's preferred fix). The only way to keep a literal `any` is to isolate that code in a file recorded in `.coding-standards-ignore` *with a reason* — which exempts the whole file — never a silent inline `any`. Reach for `unknown` first regardless.
