...
1. **Readiness check** — Python version (**3.10+**, the floor of `REQUIRED_PACKAGES`; current `tree-sitter` / `tree-sitter-javascript` wheels dropped the 3.9 wheel), Python command (`python` vs `python3`), pip availability, virtualenv detection, platform (Windows / macOS / Linux), and whether every package in `REQUIRED_PACKAGES` imports.
   - **Old-launcher fallback:** if bootstrap is *launched* by a Python below the floor (e.g. your `python3` is 3.9), it scans PATH for a 3.10+ interpreter already on the machine and **re-execs itself under that one** (same argv/cwd/env, so scope detection is unchanged). It only **blocks** when no 3.10+ Python exists anywhere — bootstrap never installs or upgrades a Python interpreter, only locates one. So a host with a 3.9 default but 3.12 also installed auto-recovers instead of failing.
