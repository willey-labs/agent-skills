...
- **A UserPromptSubmit command ends with `; exit 0`.** Exit 2 on that event erases what the user typed, and a command the shell cannot start returns exactly 2 — a missing interpreter, a moved skill dir, an install path holding a space wired by an older bootstrap. The script's own discipline of never returning non-zero protects nothing there, because the failure happens before it runs, so the guard lives in the wired command; `; exit 0` is the one form `sh`, Git Bash and PowerShell all honour. Every other event keeps its exit code: PreToolUse fails closed by design, and the Stop judge holds the turn open with exit 2. `hooks/tests/test-prompt-hook-fail-open.py` guards both installers.
